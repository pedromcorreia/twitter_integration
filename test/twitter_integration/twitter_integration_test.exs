defmodule TwitterIntegrationTest do
  ...
  test "parse_body" do
    ...
    assert Map.get(first_response, "id") == 591659
  ...
  test "build_export_tweets most_relevants" do
    ...
    assert TwitterIntegration.build_export_tweets(request, :most_relevants) == [
      %{
        ...
        retweet_count: 1,
        screen_name: "locaweb",
        text: "The TCP program is down, transmit the solid state program so we can co @locaweb ",
  ...
  test "build_export_tweets most_mentions" do
    request = build_decoded() |> Map.get("statuses")
    assert TwitterIntegration.build_export_tweets(request, :most_mentions) ==  [{"locaweb", [%{created_at: "Mon Sep 24 03:35:21 +0000 2012", favorite_count: 26, followers_count: 257, profile_url: "https://twitter.com/locaweb", retweet_count: 1, screen_name: "locaweb", text: "The TCP program is down, transmit the solid state program so we can co @locaweb ", tweet_url: "https://twitter.com/locaweb/status/591659"}, %{created_at: "Mon Sep 24 03:35:21 +0000 2012", favorite_count: 0, followers_count: 412, profile_url: "https://twitter.com/locaweb", retweet_count: 0, screen_name: "locaweb", text: "We nee @locaweb the back-end JSON bus!", tweet_url: "https://twitter.com/locaweb/status/705094"}]}]
  end
